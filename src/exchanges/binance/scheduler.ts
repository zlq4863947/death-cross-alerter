...
export function runSchedule(period: string, fn: () => void): void {
  ...
  switch (period) {
    ...
    case Resolution.hour: {
      ...
      Logger.log(`每小时计划任务，将在${moment(formattedTime).format()}开始执行！`, 'binance');
      if (time) {
    ...
    case Resolution.hour2: {
      ...
      Logger.log(`每2小时计划任务，将在${moment(formattedTime).format()}开始执行！`, 'binance');
      if (time) {
        Scheduler.min(time, fn, formattedTime);
      }
      break;
    ...
    case Resolution.hour4: {
      ...
      Logger.log(`每4小时计划任务，将在${moment(formattedTime).format()}开始执行！`, 'binance');
      if (time) {
        Scheduler.min(time, fn, formattedTime);
      }
      break;
    ...
    case Resolution.hour6: {
      ...
      Logger.log(`每6小时计划任务，将在${moment(formattedTime).format()}开始执行！`, 'binance');
      if (time) {
        Scheduler.min(time, fn, formattedTime);
      }
      break;
    }
    case Resolution.hour8: {
      const formattedTime = getFutureTime({ hour: 8, minute: 0 });
      Logger.log(`每8小时计划任务，将在${moment(formattedTime).format()}开始执行！`, 'binance');
      if (time) {
        Scheduler.min(time, fn, formattedTime);
      }
      break;
    ...
    case Resolution.hour12: {
      ...
      Logger.log(`每12小时计划任务，将在${moment(formattedTime).format()}开始执行！`, 'binance');
      if (time) {
        Scheduler.min(time, fn, formattedTime);
      }
      break;
    ...
    default: {
      Logger.warn(`未定义的计划任务时间:${period}`, 'binance');
    }
  }
}
